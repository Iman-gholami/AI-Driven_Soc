...
const Dashboard:React.FC=()=>{
 ...
 return <div style={{background:colors.bg,minHeight:'100%',padding:24,color:colors.text}}>
   ...
   <Row gutter={[16,16]} style={{marginTop:16}}>
    <Col xs={24} lg={12}><Card title='Top Detection Rules' style={{background:colors.card}}>{['WinWrapper C2 Beacon','Suspicious PowerShell','Malicious Document'].map((x,i)=><div key={x}><Text>{x}</Text><Progress percent={[75,55,35][i]}/></div>)}</Card></Col>
    <Col xs={24} lg={12}><Card title='Threat Flow' style={{background:colors.card}}><div style={{fontSize:16}}>Endpoint → Network → Detection → AI Decision</div><p style={{color:colors.muted}}>Attack chain visualization ready for telemetry integration</p></Card></Col>
   </Row>

   <Card title='Active Investigation Queue' loading={isLoading} style={{marginTop:16,background:colors.card}}><Table<Alert> rowKey='alertId' dataSource={alerts.slice(0,10)} columns={columns} pagination={false}/></Card>
