...
function comparableRecord(record) {
  // Mongo/Mongoose applies schema defaults while persisting nested objects. A freshly
  // parsed plain object can omit those same null/default fields, which previously made
  // a just-applied record look like metadata drift on the very next dry run. Hydrate
  // both sides through the schema first so reconciliation compares the persisted shape.
  const input = plain(record) || {};
  const normalized = new HistoricalReport(input).toObject({
    depopulate: true,
    flattenMaps: true,
    versionKey: false,
  });
  ...
  return {
    ...
    reportNumber: normalized.reportNumber || null,
    reportDateRaw: normalized.reportDateRaw || null,
    year: normalized.year ?? null,
    month: normalized.month ?? null,
    day: normalized.day ?? null,
    title: normalized.title || null,
    reportType: normalized.reportType || "unknown",
    provider: normalized.provider || null,
    contact: normalized.contact || null,
    effect: normalized.effect || null,
    target: plain(normalized.target),
    severity: plain(normalized.severity),
    urgency: plain(normalized.urgency),
    finding: plain(normalized.finding),
    vulnerability: plain(normalized.vulnerability),
    cves: array(normalized.cves),
    affectedCves: array(normalized.affectedCves),
    description: normalized.description || "",
    conclusion: normalized.conclusion || "",
    recommendations: array(normalized.recommendations),
    affectedSystems: array(normalized.affectedSystems).map(plain),
    phishingInfrastructure: array(normalized.phishingInfrastructure).map(plain),
    indicators: array(normalized.indicators).map(plain),
    fullText: normalized.fullText || "",
    source: {
      filename: normalized.source?.filename || null,
      relativePath: normalized.source?.relativePath || null,
      sha256: normalized.source?.sha256 || null,
      sizeBytes: normalized.source?.sizeBytes ?? 0,
    },
    extraction: {
      parserVersion: normalized.extraction?.parserVersion || null,
      paragraphCount: normalized.extraction?.paragraphCount ?? 0,
      tableCount: normalized.extraction?.tableCount ?? 0,
      warnings: array(normalized.extraction?.warnings),
      organizationMismatch: Boolean(normalized.extraction?.organizationMismatch),
      ipMismatch: Boolean(normalized.extraction?.ipMismatch),
    },
  };
}
