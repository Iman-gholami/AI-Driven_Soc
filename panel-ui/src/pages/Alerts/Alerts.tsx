...
const Alerts: React.FC = () => {
  ...
  const getSeverityIcon = (severity: string) => ({ critical: '🔴', high: '🟠', medium: '🟡', low: '🟢', info: '🔵', unknown: '⚪' }[severity] || '⚪');
  ...
  return <div>
    <div className="flex justify-between items-center mb-6 flex-wrap gap-4"><div><Title level={2} className="mb-1">Security Alerts</Title><Text type="secondary">Server-side queue backed by persisted SIEM/Splunk alerts</Text></div><Space><Button icon={<FilterOutlined />} onClick={clearFilters}>Clear Filters</Button><Button icon={<DownloadOutlined />} loading={exporting} onClick={exportToCSV}>Export Filtered CSV</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => setIsModalVisible(true)}>Ingestion Info</Button></Space></div>
    ...
    <Card className="mb-4">
      <Space wrap>
        ...
        <Text type="secondary">{pagination.total} matching alerts</Text>
      </Space>
    </Card>
