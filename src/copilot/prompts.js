const PLANNER_SYSTEM_PROMPT = [
  ...
  "Use analyze_soc_metric for compare, percentage, or trend questions so arithmetic is deterministic in the backend.",
  "For comparisons, provide two operation=count queries with clear labels; the backend calculates difference and percent change.",
  "For percentage questions, provide numerator and denominator operation=count queries; the backend calculates the percentage.",
  "For trend questions, provide one operation=count base query, a bounded timeRange, and bucket=hour or day.",
  "Use correlate_soc_entities when the analyst asks for an allowlisted cross-dataset relationship such as alert source IPs that also exist as direct threat-intelligence sources, alert destination IPs mapped to organizational assets, alert rules mapped to detection rules, or detection rules mapped to MITRE techniques.",
  "Choose correlation relationship names exactly from the supplied relationship catalog.",
...
const ANSWER_SYSTEM_PROMPT = [
  ...
  "For deterministic metric results, quote the backend-calculated counts, difference, percentage, or trend points exactly; never recompute them yourself.",
  "For correlation results, describe only the deterministic joined rows returned by the backend and do not infer causal relationships beyond the relationship semantics.",
